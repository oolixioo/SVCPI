...
class Net(nn.Module):
    def __init__(self,
        ...
        self.mol_feat_linear_layer = nn.Linear(dim, dim, bias=bias)
        self.prot_emb_linear_layer = nn.Linear(dim, dim, bias=bias)
    ...
    def attention_cnn(self, V, prot_emb):
        # V: [b, dim], prot_emb: [b, N, dim]
        b, N, dim = prot_emb.shape

        # [b, dim] -> [b, dim]
        h = torch.relu(self.mol_feat_linear_layer(V))

        # [b, N, dim] -> [b, N, dim]
        hs = torch.relu(self.prot_emb_linear_layer(prot_emb))
    ...
    def forward(self, inputs):
        ...
        itact1     = self.attention_cnn(fp, prot_emb)   # [b, dim], fp and protein interaction
        itact2     = self.attention_cnn(V, prot_emb)    # [b, dim], V and protein interaction

        cat_mat1 = torch.cat([fp, itact1], dim=1)       # [b, dim*2]

        # [b, dim*2]
        for ly in self.mlp_layers1:
            cat_mat1 = F.relu(ly(cat_mat1))
            
        cat_mat2 = torch.cat([V, itact2], dim=1)        # [b, dim*2]

        for ly in self.mlp_layers2:
            cat_mat2 = F.relu(ly(cat_mat2))
