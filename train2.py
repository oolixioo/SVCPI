...
def save_model(net, optimizer, file_path):
    h5_path = file_path
    pth_path = file_path
    if file_path.endswith('.pth'):
        h5_path = file_path.replace('pth', 'h5')
    else:
        pth_path = file_path.replace('h5', 'pth')

    torch.save({'model_state_dict': net.state_dict(),
                'optimizer_state_dict': optimizer.state_dict()
                }, pth_path)

    torch.save(net, h5_path)

def load_model_state(path, net, optimizer):
...
def copy_code_files(save_path):
    files = [sys.argv[0], 'model2.py']
    for fn in files:
        shutil.copy(fn, save_path+os.path.basename(fn))
...
def run_train(cfg, data, test_data=None, isopt=False):
    ...
    def init_model():
        ...
        net = model.Net(C, L, fpdim, n_filters_list, mlp_layers, n_head, readout_layers, dim=dim, bias=bias, fp_linear_layers=fp_linear_layers, dropout_p=dropout).to(DEVICE)
    ...
    for fold, (train_idx, dev_idx) in enumerate(itero):
        ...
        if cfg['shuffle_idx']:
            if cfg['shuffle_idx_all']:
                print('Shuffle split index all...')
                len_train = len(train_idx)
                idx = np.concatenate((train_idx, dev_idx), axis=0)

                pickle.dump(idx, open('/tmp/tmp.pkl', 'wb'))
                os.system('python shuffle.py /tmp/tmp.pkl')
                idx = pickle.load(open('/tmp/tmp.pkl', 'rb'))

                train_idx = idx[:len_train]
                dev_idx   = idx[len_train:]
        ...
        for epoch in range(cfg['max_epochs']):  # loop over the dataset multiple times
            ...
            for n, i in enumerate(range(0, len_dataset, batch_size)):
                i, j = i, i+batch_size
                data_batch = utils.create_mol_protein_batch(train_data[i:j], pad=True, device=DEVICE, pr=False, ee=cfg['ee'])
                optimizer.zero_grad()
                outputs, labels, pred_labels, pred_scores = net(data_batch)
            ...
            if test_data:
                ...
                if test_pr_auc > best_test_pr:
                    best_test_pr = test_pr_auc
                    save_model(net, optimizer, save_path+os.sep+'model-fold%s.pth' %fold)
    ...
    return save_path, best_auc, best_eval_val, best_test_pr
